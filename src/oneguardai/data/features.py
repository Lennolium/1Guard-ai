...
from oneguardai.scan import (domain_url, https_ssl, misc, registrar, review,
                             script)
...
class WebsiteFeatures:
    features_names = [
            ...
            # Script features.
            "MOUSE_OVER", "RIGHT_CLICK", "POPUP", "I_FRAME",
            # General Website/HTML-Body features (misc).
            "FAVICON", "WEBSITE_TRAFFIC", "FORWARDING",
            # Review sites features: TrustPilot.
            "TP_RATING", "TP_REVIEWS_COUNT",
            ...
            "GSO_PULSEDIVE", "GSO_QUAD9",
            # Review sites features: PageRank.
            "PR_GLOBAL_RANK", "PR_PAGE_RANK",
    ...
    def feature_extraction(self):
        ...
        self.features.append(sec_headers.get("content-security-policy",
                                             "NaN"
                                             )
                             )
        self.features.append(sec_headers.get("x-content-type-options",
                                             "NaN"
                                             )
                             )
        self.features.append(sec_headers.get("x-frame-options",
                                             "NaN"
                                             )
                             )
        self.features.append(sec_headers.get("secure-cookies",
                                             "NaN"
                                             )
                             )
        ...
        self.features.append(sa_results.get("rating",
                                            "NaN"
                                            )
                             )
        self.features.append(sa_results.get("backlinks",
                                            "NaN"
                                            )
                             )
        self.features.append(sa_results.get("website_speed",
                                            "NaN"
                                            )
                             )
        self.features.append(sa_results.get("ssl_certificate_valid",
                                            "NaN"
                                            )
                             )
