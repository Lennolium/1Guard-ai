...
def MultipleUsernames(username):
    '''replace the parameter with with symbols and return a list of
    usernames'''
    allUsernames = []
...
def sherlock(username, site_data, query_notify,
    ...
    query_notify           -- Object with base type of QueryNotify().
                              ...
                              query results.
    tor                    -- Boolean indicating whether to use a tor
    circuit for the requests.
    unique_tor             -- Boolean indicating whether to use a new tor
    ...
    # First create futures for all requests. This allows for the requests to
    # run in parallel
    for social_network, net_info in site_data.items():
        ...
        # information since they think that we are bots (Which we actually
        # are...)
        headers = {
        ...
        if regex_check and re.search(regex_check, username) is None:
            # No need to do the check at the site: this username is not
            # allowed.
        ...
        # Add this site's results into final dictionary with all the other
        # results.
        results_total[social_network] = results_site

    # Open the file containing account links
    # Core logic: If tor requests, make them here. If multi-threaded
    # requests, wait for responses
    for social_network, net_info in site_data.items():
        ...
        if error_text is not None:
            ...
            # Checks if the Status Code is equal to the optional "errorCode"
            # given in 'data.json'
...
def timeout_check(value):
    ...
    Keyword Arguments:
    value                  -- Time in seconds to wait before timing out
    request.
...
def main():
    version_string = f"%(prog)s {__version__}\n" + \
                     (f"{requests.__description__}:  "
                      f"{requests.__version__}\n") + \
                     f"Python:  {platform.python_version()}"

    parser = ArgumentParser(formatter_class=RawDescriptionHelpFormatter,
                            description=f"{module_name} (Version"
                                        f" {__version__})"
                            )
    ...
    parser.add_argument("--folderoutput", "-fo", dest="folderoutput",
                        help="If using multiple usernames, the output of the "
                             "results will be saved to this folder."
                        )
    parser.add_argument("--output", "-o", dest="output",
                        help="If using single username, the output of the "
                             "result will be saved to this file."
                        )
    parser.add_argument("--tor", "-t",
                        action="store_true", dest="tor", default=False,
                        help="Make requests over Tor; increases runtime; "
                             "requires Tor to be installed and in system path."
                        )
    parser.add_argument("--unique-tor", "-u",
                        action="store_true", dest="unique_tor", default=False,
                        help="Make requests over Tor with new Tor circuit "
                             "after each request; increases runtime; "
                             "requires Tor to be installed and in system path."
                        )
    ...
    parser.add_argument("--xlsx",
                        ...
                        help="Create the standard file for the modern "
                             "Microsoft Excel spreadsheet (xslx)."
                        )
    parser.add_argument("--site",
                        action="append", metavar="SITE_NAME",
                        dest="site_list", default=None,
                        help="Limit analysis to just the listed sites. Add "
                             "multiple options to specify more than one site."
                        )
    parser.add_argument("--proxy", "-p", metavar="PROXY_URL",
                        action="store", dest="proxy", default=None,
                        help="Make requests over a proxy. e.g. "
                             "socks5://127.0.0.1:1080"
                        )
    parser.add_argument("--json", "-j", metavar="JSON_FILE",
                        dest="json_file", default=None,
                        help="Load data from a JSON file or an online, "
                             "valid, JSON file."
                        )
    ...
    parser.add_argument("username",
                        ...
                        action="store",
                        help="One or more usernames to check with social "
    ...
    parser.add_argument("--nsfw",
                        action="store_true", default=False,
                        help="Include checking of NSFW sites from default "
                             "list."
                        )
    ...
    try:
        ...
        if remote_version != local_version:
            print("Update Available!\n" +
                  f"You are running version {local_version}. Version "
                  f"{remote_version} is available at "
                  f"https://github.com/sherlock-project/sherlock"
                  )
    ...
    if args.tor or args.unique_tor:
        ...
        print(
                ...
                "so note that using this option might increase connection "
                "errors."
                )
    ...
    if args.site_list is None:
        ...
        # User desires to selectively run queries on a sub-set of the site
        # list.
        # Make sure that the sites are supported & build up pruned site
        # database.
        site_data = {}
        site_missing = []
        for site in args.site_list:
            ...
            for existing_site in site_data_all:
                if site.lower() == existing_site.lower():
                    ...
                    counter += 1
            if counter == 0:
                # Build up list of sites not supported for future error
                # message.
                site_missing.append(f"'{site}'")

        if site_missing:
            print(
                    f"Error: Desired sites not found: "
                    f"{', '.join(site_missing)}."
                    )
    ...
    for username in all_usernames:
        ...
        if args.csv:
            ...
            with open(result_file, "w", newline='', encoding="utf-8"
                      ) as csv_report:
        ...
        if args.xlsx:
            ...
            DataFrame = pd.DataFrame({"username": usernames, "name": names,
                                      "url_main": url_main,
                                      "url_user": url_user, "exists": exists,
                                      "http_status": http_status,
                                      "response_time_s": response_time_s
                                      }
                                     )
            DataFrame.to_excel(f'{username}.xlsx', sheet_name='sheet1',
                               index=False
                               )
